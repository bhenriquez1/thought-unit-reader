...
import AutoGeneratedNotesPanel from './AutoGeneratedNotesPanel';
import NoteLabBoard from '@/components/notelab/NoteLabBoard';
import ConnectionMap from '@/components/notelab/ConnectionMap';
import { annotationsToLearningCards } from '@/lib/learning/transformers';
...
interface PureNoteLabViewProps {
  ...
  // New: for importing from Surgeon View
  currentPageHighlights?: Annotation[];
  currentPageConcepts?: Array<{ id: string; label: string; oneLiner: string; tags?: string[] }>;
}

type FilterType = 'all' | 'highlights' | 'notes' | 'flashcards' | 'weak' | 'pattern' | 'decision' | 'mnemonic' | 'absorption';
type SortType = 'recent' | 'page' | 'type';
type GroupByType = 'none' | 'chapter' | 'type' | 'topic';

type NoteLabMode = 'manual' | 'auto';
type WorkspaceMode = 'board' | 'recall' | 'connections' | 'hooks';
...
export default function PureNoteLabView({
  ...
  // View mode - manual annotations or auto-generated notes
  const [noteLabMode, setNoteLabMode] = useState<NoteLabMode>('auto');
  ...
  // Import from Surgeon View - creates NoteLab items from current page concepts and highlights
  const handleImportFromSurgeonView = useCallback(async () => {
    setImportStatus('importing');
    let importedCount = 0;

    try {
      // Get PDRM entries for current page
      const pdrmEntries = pdrmStore.getEntriesByPage(documentId, currentPage);

      // Import PDRM entries as annotations with proper PDRM metadata
      for (const entry of pdrmEntries) {
        // Check if already imported (by highlightId)
        const existingAnnotations = getAllAnnotationsArray();
        const alreadyImported = existingAnnotations.some(a =>
          a.selectedText === entry.evidence.quote &&
          a.pageIndex === entry.evidence.pageNumber - 1
        );

        if (!alreadyImported) {
          await addAnnotation({
            documentId,
            chapterId: chapters.find(c => c.pageNumber <= currentPage && (!c.endPage || c.endPage >= currentPage))?.id || 'default',
            pageIndex: currentPage - 1,
            selectedText: entry.evidence.quote,
            anchor: { type: 'textRange', start: 0, end: entry.evidence.quote.length },
            pdrm: {
              pattern: entry.fields.pattern || undefined,
              decisionRule: entry.fields.decisionRule || undefined,
              mnemonic: entry.fields.mnemonic || undefined,
            },
            color: entry.primaryType === 'pattern' ? '#9333EA' :
              entry.primaryType === 'decision' ? '#3B82F6' :
                entry.primaryType === 'mnemonic' ? '#F59E0B' : '#FFEB3B',
            tags: ['imported-from-surgeon', `pdrm-${entry.primaryType}`],
            userId
          });
          importedCount++;
        }
      ...
      // Import concepts as note-type annotations
      for (const concept of currentPageConcepts) {
        const existingAnnotations = getAllAnnotationsArray();
        const alreadyImported = existingAnnotations.some(a =>
          a.selectedText === concept.oneLiner &&
        ...
        if (!alreadyImported) {
          await addAnnotation({
            ...
            chapterId: chapters.find(c => c.pageNumber <= currentPage && (!c.endPage || c.endPage >= currentPage))?.id || 'default',
            pageIndex: currentPage - 1,
            selectedText: concept.oneLiner,
            anchor: { type: 'textRange', start: 0, end: concept.oneLiner.length },
            pdrm: {},
            color: '#10B981',
            tags: ['imported-from-surgeon', 'concept', ...(concept.tags || [])],
            noteTitle: concept.label,
            userId
          });
          importedCount++;
        }
  ...
  }, [documentId, currentPage, chapters, userId, pdrmStore, currentPageConcepts, getAllAnnotationsArray, addAnnotation]);
  ...
  const [activeLearningCardId, setActiveLearningCardId] = useState<string | null>(null);
  const [localLearningCards, setLocalLearningCards] = useState<LearningCard[]>([]);
  ...
  const groupedAnnotations = useMemo(() => {
    ...
    if (groupBy === 'chapter') {
      ...
      // Group by syllabus topics (CourseContext integration)
      const topics = courseContext.syllabusTopics;
      ...
      filteredAnnotations.forEach(ann => {
        // Check annotation's topicId first
        if (ann.topicId) {
          const topic = topics.find(t => t.id === ann.topicId);
          const groupKey = topic ? `📋 ${topic.title}` : '📁 Uncategorized';
          if (!groups[groupKey]) groups[groupKey] = [];
          groups[groupKey].push(ann);
        } else {
          // Fallback: find topic by page range
          const topic = topics.find(t =>
            t.pageRanges.some(r => ann.pageIndex >= r.start && ann.pageIndex <= r.end)
          );
          const groupKey = topic ? `📋 ${topic.title}` : '📁 Uncategorized';
          if (!groups[groupKey]) groups[groupKey] = [];
          groups[groupKey].push(ann);
  ...
  }, [filteredAnnotations, groupBy, chapters, courseContext.syllabusTopics]);
  ...
  const learningCards = useMemo(() => {
    const baseCards = annotationsToLearningCards(filteredAnnotations, documentId);
    if (!localLearningCards.length) return baseCards;
    const byId = new Map(baseCards.map((card) => [card.id, card]));
    localLearningCards.forEach((card) => byId.set(card.id, card));
    return Array.from(byId.values());
  }, [filteredAnnotations, documentId, localLearningCards]);

  const graph = useMemo(() => buildConnectionGraph(learningCards), [learningCards]);
  ...
  const handleAddToStudy = useCallback(async (cardId: string) => {
    const card = learningCards.find((entry) => entry.id === cardId);
    if (!card) return;
    await addAnnotation({
      documentId,
      chapterId: card.chapterId || 'default',
      pageIndex: Math.max(0, card.page - 1),
      selectedText: card.body,
      anchor: { type: 'textRange', start: 0, end: Math.min(card.body.length, 120) },
      pdrm: {},
      color: '#10B981',
      tags: ['study-card', `type-${card.type}`],
      flashcardFront: card.recallPrompt || card.title,
      flashcardBack: card.body,
      noteTitle: card.title,
      userId,
    });
  }, [learningCards, addAnnotation, documentId, userId]);

  const handleCreateLink = useCallback((sourceId: string, targetId: string, relation: CardRelationType) => {
    const seed = annotationsToLearningCards(filteredAnnotations, documentId);
    const base = localLearningCards.length ? localLearningCards : seed;
    setLocalLearningCards(base.map((card) => {
      if (card.id !== sourceId) return card;
      const exists = card.relations.some((entry) => entry.targetId === targetId && entry.relation === relation);
      if (exists) return card;
      return { ...card, relations: [...card.relations, { targetId, relation }] };
    }));
  }, [filteredAnnotations, documentId, localLearningCards]);
  ...
  // If auto mode with getPageText available, show AutoGeneratedNotesPanel
  if (noteLabMode === 'auto' && getPageText) {
    return (
      <div className="h-full flex flex-col bg-gray-900" data-testid="pure-notelab-view">
        {/* Mode Toggle Header */}
        <div className="flex items-center gap-2 px-4 py-2 bg-gray-800 border-b border-gray-700">
          <span className="text-sm text-gray-400">Mode:</span>
          <button
            onClick={() => setNoteLabMode('auto')}
            className="px-3 py-1 rounded text-sm font-medium transition-colors bg-purple-600 text-white"
          >
            Auto-Generated
          </button>
          <button
            onClick={() => setNoteLabMode('manual')}
            className="px-3 py-1 rounded text-sm font-medium transition-colors bg-gray-700 text-gray-300 hover:bg-gray-600"
          >
            Manual Notes
          </button>
          <span className="flex-1" />
          <span className="text-xs text-gray-500">
            Auto notes use Universal Thought-Unit extraction
          </span>
        </div>
        {/* Auto-Generated Notes Panel */}
        <div className="flex-1 overflow-hidden">
          <AutoGeneratedNotesPanel
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900" data-testid="pure-notelab-view">
      {/* Mode Toggle Header (shown when getPageText is available) */}
      {getPageText && (
        <div className="flex items-center gap-2 px-4 py-2 bg-gray-800 border-b border-gray-700">
          <span className="text-sm text-gray-400">Mode:</span>
          <button
      ...
      <div className="w-64 border-r border-gray-700 flex flex-col bg-gray-800">
        ...
        <div className="p-4 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-white mb-1">Manual Notes</h2>
        ...
        {/* Import from Surgeon View */}
        <div className="p-3 border-t border-gray-700 space-y-2">
          <button
            onClick={handleImportFromSurgeonView}
            disabled={importStatus === 'importing'}
            className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
            data-testid="import-surgeon-btn"
          >
          ...
          <p className="text-xs text-gray-500 text-center">
            Import page {currentPage} concepts & PDRM
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          <div className="flex flex-wrap gap-2">
            {([
              { key: 'board', label: 'Concept Board' },
              { key: 'recall', label: 'Recall Notes' },
              { key: 'connections', label: 'Connection Map' },
              { key: 'hooks', label: 'Memory Hooks' },
            ] as const).map((modeOption) => (
              <button
                key={modeOption.key}
                onClick={() => setWorkspaceMode(modeOption.key)}
                className={`px-3 py-1.5 rounded-lg text-sm ${workspaceMode === modeOption.key ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
              >
                {modeOption.label}
              </button>
            ))}
          ...
          {workspaceMode === 'connections' ? (
            <ConnectionMap
              nodes={graph.nodes}
              edges={graph.edges}
              activeNodeId={activeLearningCardId}
              onSelectNode={setActiveLearningCardId}
              onOpenSource={(page) => onNavigateToPage?.(page)}
            />
          ) : (
            <NoteLabBoard
              cards={learningCards}
              activeCardId={activeLearningCardId}
              onSelectCard={setActiveLearningCardId}
              onAddToStudy={handleAddToStudy}
              onOpenSource={(id) => {
                const card = learningCards.find((entry) => entry.id === id);
                if (card) onNavigateToPage?.(card.page);
              }}
              onCreateLink={handleCreateLink}
              viewMode={workspaceMode === 'recall' ? 'recall' : workspaceMode === 'hooks' ? 'hooks' : 'board'}
            />
          )}

    ...
    <div className="flex flex-col h-full bg-gray-900 text-white p-4 rounded-lg">
      ...
      {isReviewMode ? (
        <div className="bg-gray-800 p-4 rounded-lg">
          {currentCard ? (
            <>
              ...
              <button onClick={gradeCard} className="bg-blue-500 px-3 py-1 rounded">Next Card</button>
            </>
          ) : (
        ...
        <>
          ...
          <textarea rows={2} placeholder="Mnemonic (auto‑generated)"
              ...
              {localAttachments.map((link, idx) => (
                <div key={idx} className="bg-gray-800 p-2 rounded">
                  {link.includes("youtube.com") || link.includes("youtu.be") ? (
                    <iframe width="100%" height="200" src={link.replace("watch?v=", "embed/")} allowFullScreen></iframe>
                  ) : (
                    <a href={link} target="_blank" rel="noopener noreferrer" className="text-yellow-400 underline">{link}</a>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            ...
            <button onClick={handleExportFlashcard} className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded">
              📇 Export Flashcard
            </button>

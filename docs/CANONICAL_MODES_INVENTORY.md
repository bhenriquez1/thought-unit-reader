...
| `components/notelab/LearningSourcesManager.tsx` / `EvidenceWorkspace.tsx` | NoteLab | Standing Evidence/source panel for the active concept | Superseded product surface | M5 wove it inline into `UltraNotesList.tsx`; P4 (Evidence-as-provenance correction) removed it entirely — provenance now surfaces only via per-object actions (View Source/Jump to Reader/Ask Professor) on `NotebookCanvas.tsx`'s N4 action panel | canonical page evidence | REMOVE (P4: confirmed zero importers, deleted) |
| `components/workspace/PersonalWorkspaceTab.tsx` | NoteLab | Generic personal-workspace UI that displaced structured saved notes | Superseded product surface | No canonical NoteLab render | workspace store retained (still used by `components/studyguide/AdaptiveGuideView.tsx`) | REMOVE (M6: confirmed zero importers, deleted) |
...
- M6: the pre-N1 card-based NoteLab surfaces (`PureNoteLabView`, `AutoGeneratedNotesPanel`, `ConnectionMap`, `NoteLabBoard`, `AdaptiveStudySheetCard`, `DATStudySheetCard`, `NoteCardGrid`, `CardDiagramPreview`, `cognitiveProcessor.ts`, `canonicalStudyGuideBuilder.ts`, `noteAnchors.ts`, `PersonalWorkspaceTab`) had zero remaining importers after N1-N6/M1-M5 and were deleted rather than left as dead weight; `adaptiveStudySheet.ts`/`datStudySheet.ts`/`studySheetProfiles.ts` stay, since the API routes and `ultraNoteStore.ts`'s legacy-note migration still use them.

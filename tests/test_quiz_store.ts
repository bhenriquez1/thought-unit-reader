...
describe('Quiz Store Tests', () => {
  ...
  describe('Quiz Generation', () => {
    it('should generate quiz questions from highlights', async () => {
      ...
      const { useQuizStore } = await import('../lib/stores/quizStore');
      ...
      const recallQuestions = currentQuiz?.questions.filter(q => q.type === 'recall') || [];
      const applicationQuestions = currentQuiz?.questions.filter(q => q.type === 'application') || [];
  ...
  describe('Quiz Answer Submission', () => {
    it('should track correct and incorrect answers', async () => {
      ...
      if (currentQuiz && currentQuiz.questions.length > 0) {
        ...
        const answer = updatedState.currentQuiz?.answers.find(a => a.questionId === firstQuestion.id);
...
// Run tests summary
console.log('\n📋 Quiz Store Test Suite');
console.log('========================');
console.log('Tests cover:');
console.log('  - Quiz generation from highlights (3 recall, 2 application questions)');
console.log('  - Answer submission and correctness tracking');
console.log('  - Question navigation (next/prev)');
console.log('  - Quiz completion and score calculation');
console.log('  - Best score tracking per chapter');
console.log('  - localStorage persistence of attempts');
